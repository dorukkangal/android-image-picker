...
object ConfigUtils {
    ...
    fun checkConfig(config: ImagePickerConfig?): ImagePickerConfig {
        if (config == null) {
            throw IllegalStateException("ImagePickerConfig cannot be null")
        }

        if (config.mode != IpConstants.MODE_SINGLE && (config.returnMode == ReturnMode.GALLERY_ONLY || config.returnMode == ReturnMode.ALL)) {
            throw IllegalStateException("ReturnMode.GALLERY_ONLY and ReturnMode.ALL is only applicable in Single Mode!")
        }
        return config
    }
    ...
    fun getFolderTitle(context: Context, config: ImagePickerConfig): String {
        ...
        return if (folderTitle.isNullOrBlank()) {
    ...
    fun getImageTitle(context: Context, config: ImagePickerConfig): String {
        ...
        return if (configImageTitle.isNullOrBlank()) {
